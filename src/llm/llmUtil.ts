...
export async function generate(prompt: string, onStatusUpdate: StatusUpdateCallback, temperature?: number): Promise<string> {
  ...
  switch (theConnection.connectionType) {
    case LLMConnectionType.WEBLLM: message = await webLlmGenerate(theConnection, theMessages, prompt, _captureFirstResponse, temperature); break;
